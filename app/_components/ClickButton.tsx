...
export function ClickButton({ onClick, children, className = '', ...props }: ClickButtonProps) {
  ...
  return (
    <div
      ref={wrapperRef}
      className={`${styles.wrapper} ${props.disabled ? styles.disabled : ''}`}
    ...
    >
      <button
        ref={buttonRef}
        onClick={onClick}
        className={`${styles.button} ${className} ${isPressed ? styles.pressed : ''}`}
        {...props}
      >
        <div className={styles.sheen} ref={sheenRef}></div>
